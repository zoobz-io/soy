...
`Exec()` expects the UPDATE to touch exactly one row and errors otherwise. When a
single UPDATE affects a batch of rows and you want them all back, use `ExecMany()`,
which returns `[]*T`:

```go
claimed, err := jobs.Modify().
    Set("status", "processing_status").
    WhereInSubquery("id", pending).   // see the Work-Queue Claim cookbook
    ExecMany(ctx, params)
```

This is the building block for the concurrent work-queue claim — see
[Work-Queue Claim](../4.cookbook/3.work-queue.md).
